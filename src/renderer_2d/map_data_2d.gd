...
## 接入上报 own FULL → 替换本地表（单表语义：MAP_FULL=replace；多车合并暂缓，后置）

func set_full(vehicle_id: String, chunk_x: int, chunk_y: int, cells: PackedByteArray) -> void:
	...
	chunk.cells = incoming
	# DEBUG
	var c := _count_states(chunk.cells)
	print("[MapData2D] set_full: veh=%s chunk(%d,%d) [free:%d occupied:%d unknown:%d]" % [vehicle_id, chunk_x, chunk_y, c[0], c[1], c[2]])
	# 显示刷新
