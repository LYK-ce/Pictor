...
## 链路验证：hello → pose(帧) → map_full(帧) → manual_control → task_set(队列替换) → cancel
	...
	match _state:
		...
		1:  # 收 hello/pose/map → 发 manual
			...
			if _manual_moving:
				_state = 2
				_phase_t = 0.0
				print("[TEST] manual OK, vehicle moving — sending task_set x2")
		...
		2:  # 等待任务执行
			_Poll_Client()
			if _task_running and _phase_t > 6.0 and not _sent_cancel:
				_sent_cancel = true
...
func _Finish() -> void:
	...
	print("hello=", _got_hello, " map=", _map_ok, " manual_move=", _manual_moving,
		" task=", _task_running, " cancel_stop=", _cancel_stopped, " poses=", _pose_count)
